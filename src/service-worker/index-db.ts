...
import { cleanupOldLogs, exportLogs, getRecentLogs } from './logger';
import { SEED_COMMENTS, SEED_EVIDENCE, SEED_JOURNEYS, SEED_TASKS } from './seed';
 ...
 */
export async function handleDebug(method: string, input: any): Promise<any> {
  switch (method) {
    case 'getRecentLogs':
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onsuccess = () => {
      ...
      console.log('[SW] Auth session loaded:', {
        ...
        hasSupabaseToken,
        supabaseTokenExpiry: session.supabaseExpiresAt
          ? new Date(session.supabaseExpiresAt).toISOString()
          : 'none',
      });
...
export async function patchMetadata(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Failed to patch metadata: ${response.status} ${errorData.error || response.statusText}`
      );
    }
